...
export default function DebriefScreen({ debrief, isLoading, error, onRetry, onReset, scenario }) {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0B] flex flex-col text-[#E8E6E1] overflow-x-hidden">
      ...
      <nav className="relative z-20 flex items-center justify-between px-12 py-8 border-b border-[#1E1E22]">
        <div className="flex items-center gap-4">
          <div className="w-6 h-6 rounded bg-[#FF4B1F] flex items-center justify-center">
            <svg width="12" height="12" viewBox="0 0 14 14" fill="none"><path d="M7 1C7 1 9 4 9 6.5C9 7.88 7.88 9 6.5 9C5.12 9 4 7.88 4 6.5C4 5.5 4.5 4.5 5 4C4 5 3 7 4.5 9.5C5.3 11 6.5 12 6.5 12C6.5 12 10 10 10 6.5C10 3.5 7 1 7 1Z" fill="white" /></svg>
          </div>
          <span className="font-display text-lg tracking-[0.2em]">VERDICT_ENGINE</span>
        </div>
        <button onClick={onReset} className="text-[10px] font-mono uppercase tracking-[0.3em] px-5 py-2 rounded-full border border-[#1E1E22] hover:bg-[#FF4B1F] transition-all">Reset Simulator</button>
      </nav>

      <div className="relative z-10 flex-1 flex flex-col max-w-6xl mx-auto w-full px-12 py-16">
        <div className={`transition-all duration-1000 ${ready ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          <header className="mb-20 text-center max-w-3xl mx-auto">
            <span className="text-[10px] font-mono uppercase tracking-[0.6em] text-orange-500/60 mb-6 block">Final Assessment</span>
            <h1 className="font-display text-8xl leading-none mb-4 tracking-tight">{styleName.toUpperCase()}</h1>
            <p className="text-xl font-body text-[#6B6B70] italic mb-6 leading-relaxed">"{styleSubtitle}"</p>
          </header>

          <div className="grid lg:grid-cols-[1fr_2fr] gap-16 items-start">
            <section className="bg-[#111113]/50 rounded-3xl p-8 border border-[#1E1E22]">
              <h3 className="text-[10px] font-mono uppercase tracking-[0.4em] mb-10 text-[#3A3A3F]">Judgment_Neural_Map</h3>
            ...
            <div className="grid md:grid-cols-2 gap-6 h-full">
              ...
              <button onClick={onReset} className="md:col-span-2 mt-4 py-6 rounded-2xl bg-[#FF4B1F] text-white font-mono text-xs uppercase tracking-[0.4em] hover:brightness-110 transition-all shadow-xl shadow-orange-900/20">Restart Neural Protocol</button>
            </div>
          </div>
        </div>
      </div>
